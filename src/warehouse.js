...
export const entries = [
  ...
  { id: 'voice', root: 'lab', rootLabel: 'research lab', label: 'voice AI', lenses: ['AI', 'ML'],
    ...
    narrative: "A separate, ongoing engagement alongside the FHI internship — voice-AI research at UW–Madison. I'm co-authoring the voice layer of SteMy, a hands-free AI lab assistant the People and Robots Lab (Prof. Bilge Mutlu) is building with the Kamp Lab (Dr. Timothy Kamp) for bench scientists: OpenAI Realtime full-duplex speech, function-calling into lab tools, and a spoken confirmation gate before any real action. A ~9-person team; my part is the voice and tool-dispatch flow. The work is part of an active NIH R01 proposal that scored highly — funding anticipated.",
    skills: ['Voice AI (OpenAI Realtime)', 'Function-calling', 'Human-in-the-loop safety', 'Real-time systems', 'Research engineering'],
    tech: [
      "Architecture — React 18 + Vite front-end, Python/FastAPI + SQLite back-end, protocol-aware (it knows the differentiation day and which variables matter). The hard state lives in the backend, not the model: slot-filling for multi-turn commands, a confirmation gate on every mutating action, and a navigate_to tool that resolves 'open my Day 3 experiment, replicate 2' from any page — so the LLM has one narrow job per turn.",
      "Gen 1 → Gen 2 — migrated from a turn-based STT → intent-router → TTS pipeline (~1.8–3s) to OpenAI's Realtime API over a persistent WebRTC session (~600–900ms, native barge-in) without a rewrite: the Realtime path routes every function call back through the existing tool executor, auth, and telemetry. Shipped behind a runtime feature flag so a bench user can roll back from devtools without a redeploy.",
      "Safety & tests — a spoken confirmation gate (dry-run preview + yes/no) before any destructive lab action; a Vitest suite over the WebRTC client + an HTTP smoke test over the executor (which caught a stale-session overwrite bug before release).",
      "Scope & funding — a UW–Madison collaboration (People and Robots Lab × Kamp Lab); I build and lead the voice/tools layer while teammates build the agent and the reproducibility 'brain.' Part of an active NIH R01 proposal that scored highly — funding anticipated.",
    ],
