// Build the FHI-warehouse lineage DAG as an SVG string.
// ONLY the work that actually runs on the warehouse is drawn here: the 'warehouse'
// root at center with its children. Entries that belong to a different system
// (e.g. the research-lab voice agent, root !== 'warehouse') are intentionally
// EXCLUDED — they are queryable elsewhere but must never appear on this graph.
// Every node has data-id so app.js can fire that entry's query.
export function buildGraph(entries, opts = {}) {
  const W = opts.w || 900, H = opts.h || 340, cx = W / 2, cy = H / 2, R = opts.r || 120;
  const children = entries.filter(e => e.id !== 'warehouse' && (e.root || 'warehouse') === 'warehouse');
  const pts = children.map((e, i) => {
    const a = (-Math.PI / 2) + (i * 2 * Math.PI / children.length);
