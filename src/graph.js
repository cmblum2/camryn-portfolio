...
export function buildGraph(entries, opts = {}) {
  ...
  sources.forEach((s, i) => {
    ...
    srcEdges += `<line x1="${sx}" y1="${y}" x2="${whX}" y2="${whY}" stroke="#dcdcdc" stroke-width="1"/>`;
    srcNodes += `<g><circle cx="${sx}" cy="${y}" r="4" fill="#ffffff" stroke="#bcbcbc" stroke-width="1.2"/>`
      + `<text x="${sx - 12}" y="${y + 3}" text-anchor="end" ${MONO(8)} fill="#8a8a8a">${esc(s)}</text></g>`;
  });
  ...
  groups.forEach((g, gi) => {
    ...
    const labelColor = g.dashed ? '#a8a8a8' : '#111111';
    ...
    g.ids.forEach((id, j) => {
      ...
      xs.push(x);
      const edge = g.dashed
        ? `<line x1="${whX}" y1="${whY}" x2="${x}" y2="${y}" stroke="#d0d0d0" stroke-width="1" stroke-dasharray="4 4"/>`
        : `<line x1="${whX}" y1="${whY}" x2="${x}" y2="${y}" stroke="#d0d0d0" stroke-width="1"/>`;
      prodEdges += edge;
    ...
    if (g.chain) {
      for (let k = 0; k < xs.length - 1; k++) {
        ...
        prodEdges += `<line x1="${xs[k] + 9}" y1="${gy}" x2="${xs[k + 1] - 9}" y2="${gy}" stroke="#111111" stroke-width="1.3"/>`;
        prodNodes += `<text x="${mx}" y="${gy + 3.5}" text-anchor="middle" ${MONO(9)} fill="#111111">▸</text>`;
      }
    }
  });
  ...
  const wh = `<g class="gnode" data-id="warehouse" style="cursor:pointer">`
    + `<circle cx="${whX}" cy="${whY}" r="18" fill="#111111"/>`
    + `<text x="${whX}" y="${whY + 37}" text-anchor="middle" ${MONO(11)} fill="#8a8a8a">warehouse · built from scratch</text></g>`;

  // detached band: a genuinely separate domain (research-lab voice AI), not on the warehouse
  const bandY = 452;
  let beyondSvg = `<line x1="60" y1="${bandY}" x2="940" y2="${bandY}" stroke="#d0d0d0" stroke-width="1" stroke-dasharray="4 4"/>`
    + `<text x="60" y="${bandY - 10}" ${MONO(10)} fill="#8a8a8a" letter-spacing="1">SEPARATE ENGAGEMENT — UW PEOPLE &amp; ROBOTS × KAMP LAB · VOICE AI · NIH R01</text>`;
  beyond.forEach((id, i) => {
    const e = byId[id]; if (!e) return;
...
function projNode(x, y, id, label, tag, muted) {
  const t = tag ? `<text x="${x}" y="${y + 33}" text-anchor="middle" ${MONO(8)} fill="#8a8a8a">${esc(tag)}</text>` : '';
  const stroke = muted ? '#a8a8a8' : '#111111';
  return `<g class="gnode" data-id="${id}" style="cursor:pointer">`
    + `<circle cx="${x}" cy="${y}" r="8" fill="#ffffff" stroke="${stroke}" stroke-width="1.5"/>`
    + `<text x="${x}" y="${y + 20}" text-anchor="middle" ${MONO(9.5)} fill="#222222">${esc(label)}</text>${t}</g>`;
}
