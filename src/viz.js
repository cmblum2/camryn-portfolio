...
const A = '#111111', G = '#111111', RED = '#9a9a9a', LINE = '#d8d8d8', DIM = '#8a8a8a', INK = '#111111', BLU = '#9a9a9a';
...
function frame(inner, label) {
  return `<svg viewBox="0 0 340 150" width="100%" style="display:block" role="img" aria-label="${esc(label)}">`
    + `<rect x="0.5" y="0.5" width="339" height="149" rx="10" fill="#fafafa" stroke="${LINE}"/>${inner}</svg>`;
}
const ln = (x1, y1, x2, y2, dash) => `<line x1="${x1}" y1="${y1}" x2="${x2}" y2="${y2}" stroke="${LINE}" stroke-width="1"${dash ? ' stroke-dasharray="3 3"' : ''}/>`;
const dot = (x, y, r, stroke) => `<circle cx="${x}" cy="${y}" r="${r}" fill="#ffffff" stroke="${stroke || A}" stroke-width="1.4"/>`;
...
function bars() {
  ...
  s += `<rect x="86" y="${base - 52}" width="46" height="52" rx="2" fill="rgba(0,0,0,.08)" stroke="${G}"/>`;
  s += `<rect x="200" y="${base - 90}" width="46" height="90" rx="2" fill="rgba(0,0,0,.045)" stroke="${RED}"/>`;
...
function funnel() {
  ...
  s += `<rect x="22" y="46" width="52" height="66" rx="3" fill="rgba(0,0,0,.05)" stroke="${LINE}"/>` + tx(48, 82, 'comments', 6.5, DIM, 'middle') + tx(48, 128, '(scraped)', 6, DIM, 'middle');
  // buy-intent ML model
  s += ln(74, 79, 104, 79) + `<rect x="104" y="60" width="72" height="38" rx="4" fill="rgba(0,0,0,.07)" stroke="${A}"/>` + tx(140, 76, 'buy-intent', 7, INK, 'middle') + tx(140, 88, 'lexicon', 6.5, DIM, 'middle');
  s += ln(176, 79, 204, 79);
  // ranked creators (descending score bars, top = amber)
  const bw = [62, 48, 36, 26];
  bw.forEach((w, i) => { const y = 52 + i * 17; s += `<rect x="216" y="${y}" width="${w}" height="10" rx="2" fill="${i === 0 ? 'rgba(0,0,0,.10)' : 'none'}" stroke="${i === 0 ? A : DIM}"/>` + dot(210, y + 5, 2.6, i === 0 ? A : DIM); });
...
function curve() {
  ...
  s += `<polygon points="30,122 ${pts} 316,122" fill="rgba(0,0,0,.06)"/>`;
...
function tiles() {
  ...
  bh.forEach((h, i) => { const x = 40 + i * 30; const bug = i === 2; s += `<rect x="${x}" y="${128 - h}" width="18" height="${h}" rx="2" fill="${bug ? 'rgba(0,0,0,.05)' : 'none'}" stroke="${bug ? RED : LINE}"/>`; });
  s += ln(34, 128, 190, 128);
